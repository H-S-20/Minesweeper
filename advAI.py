...
class AICell:
    def __init__(self, x, y, status, dim, map, ai_cells):
        ...
        self.unknown_neighbor_mines = self.near_mines - len(self.identified_mines)
...
class AdvAIBrain:
    ...
    def perform_query(self):
        ...
        if len(self.moves) == 0:  # After assessing all knowns, no hidden cell can be marked as a mine or safe
            ...
            pick = None
            uncovered_list = self.generate_uncovered_list()
            most_neighbors = 0
            for i in range(len(uncovered_list)):
                curr_cell = uncovered_list[i]

                if len(curr_cell.hidden_neighbors) >= most_neighbors and curr_cell.status == 2:
                    most_neighbors = len(curr_cell.hidden_neighbors)
                    pick = curr_cell

            if pick is None:
                self.pick_random_cell()
            else:
                print("Making smarter pick: " + str(pick.x) + " " + str(pick.y))
                self.check_grid(pick.x, pick.y)
        else:
            print("Moves:", end="")
            print(self.moves)
    ...
    # returns list of still uncovered cells
    def generate_uncovered_list(self):
        uncovered = []
        for i in range(len(self.ai_cells)):
            for j in range(len(self.ai_cells[i])):
                curr_cell = self.ai_cells[i][j]
                if curr_cell.status == 2:
                    uncovered.append(curr_cell)
        return uncovered
